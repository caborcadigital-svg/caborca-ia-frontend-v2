...
import { Send, User, Plus, Loader2, Trash2, Mic, MicOff } from 'lucide-react';
...
export default function ChatPage() {
  ...
  const toggleVoz = () => {
    if (!voiceSupported) { toast.error('Tu navegador no soporta voz'); return; }
    if (isListening) {
    ...
    recognition.onerror = () => { setIsListening(false); toast.error('No se pudo escuchar'); };
          ...
          <div className="shrink-0 border-t" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
            ...
            <div className="p-3" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 72px)' }}>
              <div className="flex gap-2 items-end max-w-2xl mx-auto">
                ...
                <div className="flex-1 rounded-2xl flex items-end gap-2 px-4 py-3 border shadow-sm" style={{ background: 'var(--sand)', borderColor: isListening ? '#DC2626' : 'var(--border)' }}>
                  <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                    placeholder={isListening ? 'Escuchando...' : 'Preguntame algo sobre Caborca...'}
                    rows={1} className="flex-1 bg-transparent text-sm resize-none outline-none max-h-28"
