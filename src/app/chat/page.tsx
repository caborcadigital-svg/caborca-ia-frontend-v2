    ...
    <MainLayout>
      <div className="flex h-[calc(100vh-52px)] lg:h-screen">
        {isAuthenticated && conversaciones.length > 0 && (
          <div className="hidden md:flex flex-col w-52 border-r bg-white/70" style={{ borderColor: 'var(--border)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
              {conversaciones.map(c => (
                <button key={c.id}
                  ...
                  className="w-full text-left px-3 py-2 rounded-xl text-xs transition-colors truncate hover:bg-sand-dark"
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center px-4 animate-fade-in">
                <div className="mb-4"><LogoIcon size={64} /></div>
                ...
                <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                  {SUGERENCIAS.map(s => (
                    <button key={s} onClick={() => enviar(s)}
                      className="bg-white rounded-xl p-3 text-left text-xs border border-gray-100 shadow-sm hover:shadow-md transition-all"
                      style={{ color: 'var(--text-secondary)' }}>
                      {s}
                    </button>
                  ))}
            ...
            {messages.map(msg => (
              <div key={msg.id} className={clsx('flex gap-2 animate-slide-up', msg.rol === 'user' ? 'justify-end' : 'justify-start')}>
                {msg.rol === 'assistant' && (
                  <div className="shrink-0 mt-0.5"><LogoIcon size={28} /></div>
                )}
                <div className={clsx(
                  'max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed',
                  msg.rol === 'user'
                    ? 'gradient-sunset text-white rounded-tr-sm shadow-md'
                    : 'bg-white border border-gray-100 shadow-sm rounded-tl-sm prose-chat'
          ...
          <div className="p-3 border-t bg-white/80 backdrop-blur-sm" style={{ borderColor: 'var(--border)' }}>
