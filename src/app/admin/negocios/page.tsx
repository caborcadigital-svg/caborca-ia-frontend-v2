...
const FORM_INIT = { nombre: '', categoria: 'restaurante', descripcion: '', direccion: '', telefono: '', horario: '', imagen_url: '', destacado: false };
...
export default function AdminNegociosPage() {
  ...
  const handleEdit = (n: any) => {
    setForm({ nombre: n.nombre, categoria: n.categoria || 'restaurante', descripcion: n.descripcion || '', direccion: n.direccion || '', telefono: n.telefono || '', horario: n.horario || '', imagen_url: n.imagen_url || '', destacado: n.destacado || false });
      ...
      <div className="max-w-3xl mx-auto px-3 py-4 space-y-4">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl p-4 border shadow-sm space-y-3 animate-slide-up" style={{ borderColor: 'var(--border)' }}>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl border" style={{ borderColor: 'var(--border)', background: form.destacado ? '#FEF3EB' : '#FAFAFA' }}>
              ...
              <div className="flex-1">
                ...
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>Aparece en la sección destacados del inicio</div>
              </div>
              <button onClick={() => set('destacado', !form.destacado)}
                className="w-10 h-6 rounded-full transition-all relative"
                style={{ background: form.destacado ? '#E8823A' : '#D0D0D0' }}>
                <div className="w-5 h-5 rounded-full bg-white absolute top-0.5 transition-all shadow-sm"
                  style={{ left: form.destacado ? '18px' : '2px' }} />
              </button>
            </div>
        ...
        <div className="flex gap-2">
          {(['todos', 'destacados'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-all border ${tab === t ? 'gradient-sunset text-white border-transparent shadow-sm' : 'bg-white'}`}
            ...
            {lista.map(n => (
              <div key={n.id} className={`bg-white rounded-2xl p-4 border shadow-sm flex items-center gap-3 ${n.destacado ? 'border-orange-200' : 'border-gray-100'}`}>
                {n.imagen_url ? (
                  ...
                  <div className="w-14 h-14 rounded-xl shrink-0 flex items-center justify-center" style={{ background: '#EEF4F9' }}>
