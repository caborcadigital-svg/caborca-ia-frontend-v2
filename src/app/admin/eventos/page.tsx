  ...
  return (
    <MainLayout>
      <div className="max-w-3xl mx-auto px-3 py-4 space-y-4">
        ...
        {showForm && (
          <div className="rounded-2xl p-4 border shadow-sm space-y-3 animate-slide-up" style={{ background:'var(--surface)', borderColor:'var(--border)' }}>
            ...
            <div>
              <label className="text-xs mb-2 block font-medium" style={{ color:'var(--text-secondary)' }}>Descripcion</label>
              <RichEditor value={form.descripcion} onChange={v => set('descripcion', v)} placeholder="Descripcion del evento..." height={200} />
            </div>
        ...
        <div className="space-y-2">
          {eventos.length === 0 && (
            <div className="rounded-2xl p-8 text-center border shadow-sm text-sm" style={{ background:'var(--surface)', borderColor:'var(--border)', color:'var(--text-muted)' }}>No hay eventos registrados</div>
          )}
          {eventos.map(e => (
            <div key={e.id} className="rounded-2xl p-4 border shadow-sm flex items-center gap-3" style={{ background:'var(--surface)', borderColor:'var(--border)' }}>
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => handleEdit(e)} className="w-8 h-8 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center border border-blue-100">
                  <Edit className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => handleDelete(e.id)} className="w-8 h-8 rounded-xl bg-red-50 text-red-500 hover:bg-red-600 hover:text-white transition-all flex items-center justify-center border border-red-100">
