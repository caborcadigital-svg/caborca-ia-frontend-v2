...
export default function GlobalSearch() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 flex items-start justify-center pt-16 px-3" style={{ background:'rgba(0,0,0,0.5)', zIndex:70 }}>
                ...
                <div>
                  ...
                  {resultados.noticias.map((n: any) => (
                    <button key={n.id} onClick={() => ir('/noticias/' + n.id)}
                      className="w-full flex items-center gap-3 px-4 py-3 text-left border-b" style={{ borderColor:'var(--border)' }}>
                ...
                <div>
                  ...
                  {resultados.negocios.map((n: any) => (
                    <button key={n.id} onClick={() => ir('/negocios/' + n.id)}
                      ...
                      <div><div className="text-sm" style={{ color:'var(--text-primary)' }}>{n.nombre}</div><div className="text-xs capitalize" style={{ color:'var(--text-muted)' }}>{n.categoria}</div></div>
                    </button>
                  ))}
                </div>
              )}
                ...
                <div>
                  ...
                  {resultados.eventos.map((e: any) => (
                    <button key={e.id} onClick={() => ir('/eventos')}
                      ...
                      <div><div className="text-sm" style={{ color:'var(--text-primary)' }}>{e.nombre}</div><div className="text-xs" style={{ color:'var(--text-muted)' }}>{new Date(e.fecha_inicio).toLocaleDateString('es-MX')}</div></div>
                    </button>
                  ))}
                </div>
              )}
              {!q && (
                <div className="p-4 text-center text-xs" style={{ color:'var(--text-muted)' }}>Escribe para buscar o usa el micrófono</div>
              )}
            </div>
          </div>
        </div>
