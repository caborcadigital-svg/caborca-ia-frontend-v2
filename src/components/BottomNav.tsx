...
const NAV = [
  ...
  { href: '/chat', label: 'Chat IA', icon: MessageSquare, fab: true },
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden"
      style={{ background: 'white', borderTop: '0.5px solid #E0E0E0', paddingBottom: 'env(safe-area-inset-bottom)' }}>
        ...
        {NAV.map(({ href, label, icon: Icon, fab }) => {
          ...
          if (fab) {
            return (
              <Link key={href} href={href}
                ...
                <span className="text-xs mt-1 font-medium" style={{ color: active ? 'var(--sunset-coral)' : 'var(--text-muted)', fontSize: '10px' }}>
                  {label}
                </span>
              </Link>
            );
          ...
          return (
            <Link key={href} href={href}
              ...
              {active && (
                <div className="w-1 h-1 rounded-full" style={{ background: 'var(--sunset-coral)' }} />
              )}
            </Link>
          );
        })}
